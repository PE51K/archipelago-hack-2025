...
VAL_GTCSV  = Path("data/val/val_gt.csv")
...
def sliced_wbf_predict(im, model, conf, overlap, wbf_iou):
    ...
    return [dict(
        ...
        label=0,
        score=s
    ) for (x1, y1, x2, y2), s in zip(bxs, scs)]
...
def run_validation(model, conf, overlap, wbf_iou):
    ...
    for p in VAL_IMGDIR.glob("*.*"):
        ...
        if preds:
            for d in preds:
                d |= dict(image_id=img_id, time_spent=dt,
                          w_img=im.shape[1], h_img=im.shape[0])
                rows.append(d)
        else:
            rows.append(dict(image_id=img_id, xc=None, yc=None, w=None, h=None,
                             label=0, score=None, time_spent=dt,
                             w_img=im.shape[1], h_img=im.shape[0]))
    return pd.DataFrame(rows, columns=[
        "image_id","label","xc","yc","w","h",
        "w_img","h_img","score","time_spent"
    ])
...
def fbeta(df_pred):
    ...
    score, *_  = metric.evaluate(pred_bytes, gt_bytes, beta=BETA,
                                 parallelize=False)
    return score
